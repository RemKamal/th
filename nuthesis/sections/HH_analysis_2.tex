...
Several lepton efficiencies need to be measured: the efficiency to reconstruct a lepton in the inner tracker detector, the efficiency to pass the identification selection, the isolation requirement, the efficiency of the HLT trigger. For the trigger efficiencies, as explained in section \ref{sec:TnP}, the TnP technique was used. Additionally, since the DZ requirement was absent for some trigger, the efficiency of the DZ selection was measured separately. The SF calculation for the DZ requirement is very similar to how tracker, ID, and ISO SFs are computed, e.g., the numerator contains events that pass the DZ requirement and the denominator is equal to the number of events that pass the selection without DZ requirement. All these multiplicative factors are put together to form the per-candidate lepton efficiency SF.
